<x-app-layout>
    @section('title', 'Editar Compra - JVJ Pizzería')

    <div class="max-w-xl mx-auto px-4 py-8 bg-white shadow-md rounded-lg">
        <h1 class="text-2xl font-semibold text-gray-800 mb-6">Editar Compra</h1>
    
        <form method="POST" action="{{ route('purchases.update', ['purchase' => $purchase->id]) }}">
            @method('PUT')
            @csrf
    
            {{-- ID de la compra (solo lectura) --}}
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Código</label>
                <input type="text" value="{{ $purchase->id }}" disabled
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm bg-gray-100 text-gray-600">
                <p class="text-xs text-gray-500 mt-1">ID de la compra (no editable).</p>
            </div>
                ...
                <select id="raw_material_id" name="raw_material_id" required
                    ...
                    @foreach($rawMaterials as $material)
                        <option value="{{ $material->id }}" 
                                ...
                                {{ $purchase->raw_material_id == $material->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <div class="flex items-center">
                    <input type="number" id="quantity" name="quantity" step="0.01" min="0.01" 
                           ...
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    <span id="unit-display" class="ml-2 text-gray-500">
                        {{ $purchase->rawMaterial->unit ?? '' }}
                    </span>
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="relative mt-1 rounded-md shadow-sm">
                    ...
                    <input type="number" id="purchase_price" name="purchase_price" step="0.01" min="0.01"
